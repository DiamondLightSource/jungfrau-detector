...
class JungfrauMetaWriter(MetaWriter):
    ...
    def _define_detector_datasets(self):
        return [
            ...
            Float64HDF5Dataset(
                dectris(DETECTOR_DISTANCE), cache=False, attributes=METERS
            ),
            StringHDF5Dataset(
                dectris(DETECTOR_NUMBER), encoding="ascii", length=100, cache=False
            ),
            Float64HDF5Dataset(
                dectris(DETECTOR_READOUT_TIME), cache=False, attributes=SECONDS
            ),
            Float64HDF5Dataset(
                dectris(DETECTOR_TRANSLATION), cache=False, attributes=METERS
            ),
            StringHDF5Dataset(
            ...
            Int64HDF5Dataset(
                dectris(FRAME_COUNT_TIME), cache=False, attributes=SECONDS
            ),
            ...
            Float64HDF5Dataset(
                dectris(KAPPA_INCREMENT), cache=False, attributes=DEGREES
            ),
            ...
            Float64HDF5Dataset(
                dectris(OMEGA_INCREMENT), cache=False, attributes=DEGREES
            ),
            ...
            Float64HDF5Dataset(
                dectris(PHOTON_ENERGY), cache=False, attributes=ELECTRON_VOLTS
            ),
            ...
            Float64HDF5Dataset(
                dectris(SENSOR_THICKNESS), cache=False, attributes=METERS
            ),
            StringHDF5Dataset(
                dectris(SOFTWARE_VERSION), encoding="ascii", length=100, cache=False
            ),
            Float64HDF5Dataset(
                dectris(THRESHOLD_ENERGY), cache=False, attributes=ELECTRON_VOLTS
            ),
            StringHDF5Dataset(dectris(TRIGGER_MODE), length=4, cache=False),
            Float64HDF5Dataset(
                dectris(TWO_THETA_INCREMENT), cache=False, attributes=DEGREES
            ),
            Float64HDF5Dataset(
                dectris(TWO_THETA_START), cache=False, attributes=DEGREES
            ),
            ...
            Int64HDF5Dataset(
                dectris(X_PIXELS_IN_DETECTOR), cache=False, attributes=PIXELS
            ),
            Float64HDF5Dataset(dectris(Y_PIXEL_SIZE), cache=False, attributes=METERS),
            Int64HDF5Dataset(
                dectris(Y_PIXELS_IN_DETECTOR), cache=False, attributes=PIXELS
            ),
        ]
    ...
    def handle_global_header_none(self, header, _data):
        ...
        if not self.file_open:
            self._logger.warning(
                "%s | File not open for jungfrau start message. Creating now.",
                self._name,
            )
